...
public class DragManager : MonoBehaviour
{
    ...
    private void HandleDragStart()
    {
        ...
        // 드래그 오프셋을 0으로 설정하여 터치 위치가 중앙이 되도록 함
        dragState.DragOffset = Vector3.zero;
        ...
        // 드래그 시작 시 오브젝트를 터치 위치로 즉시 이동
        if (isUI)
        {
            dragState.Target.GameObject.transform.position = (Vector3)pointerPosition;
        }
        else
        {
            Vector3 worldPos = MainCamera.ScreenToWorldPoint(pointerPosition);
            worldPos.z = 0;
            dragState.Target.GameObject.transform.position = worldPos;
        }
